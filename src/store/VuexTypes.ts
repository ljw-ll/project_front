export const LOGIN = "LOGIN";
export const UPDATE_USER = "UPDATE_USER";
export const LIST_COURSES = "LIST_COURSES";
export const LIST_GITHUBREPOS = "LIST_GITHUBREPOS";
export const LIST_USER_COURSES = "LIST_USER_COURSES";

export const GET_HOME = "GET_HOME";
export const UPDATE_EXCEPTION = "GET_EXCEPTION";

// getters
export const GETTER_PREMISSION = "GETTER_PREMISSION";

// // homework02
// export const LIST_SHOPS = "LIST_SHOPS";
// export const GET_SHOP = "GET_SHOP";

// Integrating with backend
export const BACKEND_WELCOME = "BACKEND_WELCOME";
export const BACKEND_LOGIN = "BACKEND_LOGIN";
export const BACKEND_COURSES = "BACKEND_COURSES";

// admin 操作 teacher 表
export const LIST_TEACHERS = "LIST_TEACHERS";
export const DEL_USERID = "DEL_USERID";
export const SAVE_USER = "SAVE_USER";
export const UPDATE = "UPDATE";

// admin 操作 laboratory 表
export const LIST_LABORATORY = "LIST_LABORATORY";
export const DEL_LABORATORYID = " DEL_LABORATORYID";
export const SAVE_LABORATORY = " SAVE_LABORATORY";
export const UPDATE_LABORATORY = " UPDATE_LABORATORY"; // 使用右侧的方法名， 调用左侧 方法

// 教师 操作 course 表
export const LIST_COURSE = "LIST_COURSE";
export const DEL_COURSEID = "DEL_COURSEID";
export const SAVE_COURSE = "SAVE_COURSE";
export const UPDATE_COURSE = "UPDATE_COURSE";
